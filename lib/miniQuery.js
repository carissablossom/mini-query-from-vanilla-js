    ...
    hide: function(item){
      ...
      if (domelement.length > 1) {
        for (var i = 0; i < domelement.length; i++) {
          domelement[i].style.display = "hidden";
        };
      }
      else {
...
var EventDispatcher = (function() {
  return {
    on: function(target, newEvent, newFunction) {
      var element = SweetSelector.select(target);
      element.addEventListener(newEvent, newFunction);
    },
    trigger: function(target, newEvent) {
      var event = document.createEvent('Event')
      event.initEvent(newEvent, true, true)
      SweetSelector.select(target).dispatchEvent(event)
    },
  };
})();
